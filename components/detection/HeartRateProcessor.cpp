...
namespace pscd
{
    ...
    /// @brief constructor
    pscd::detection::HeartRateProcessor::HeartRateProcessor()
    ...
    double pscd::detection::HeartRateProcessor::update(int rawIR)
    {
        dcPush(rawIR);
        ...
        if (was_rising_ && !rising && prev_ac_ >= MIN_AC_HEIGHT && beat_gap_ >= MIN_BEAT_GAP)
        {
    ...
    double pscd::detection::HeartRateProcessor::computeRMSSD() const
    {
        if (rr_count_ < BEATS_TO_AVG)
            return 0.0;
    ...
    double pscd::detection::HeartRateProcessor::computeSDNN() const
    {
        if (rr_count_ < BEATS_TO_AVG)
        {
            return 0.0;
    ...
    void pscd::detection::HeartRateProcessor::dcPush(int x)
    {
        if (dc_count_ == DC_WIN)
        {
    ...
    int32_t pscd::detection::HeartRateProcessor::dcRemove(int x) const
    {
        if (dc_count_ == 0)
        {
            return 0;
